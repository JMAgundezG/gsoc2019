...
function sysCall_threadmain()
    ...
    while sim.getSimulationState()~=sim.simulation_advancing_abouttostop do
        ...
        if (opMode==2) then
            if (c == nil) then
                c = sim.auxiliaryConsoleOpen("consola", 10, 5)
                sim.auxiliaryConsolePrint(c, 'START\n')

            end
            sim.setStringSignal('EPUCK_PROXSENS', sim.packFloatTable(proxSensDist))
            sim.auxiliaryConsolePrint(c, 'PROXSENS\n')
            sim.auxiliaryConsolePrint(c, proxSensDist[1])
            tmpLED = sim.getStringSignal('EPUCK_LED')
            if (tmpLED ~= '') then
                --sim.auxiliaryConsolePrint(c, tmpLED)    
                tmpLED = sim.unpackInt32Table(tmpLED)
                tmpLEDNum = tmpLED[1]
                tmpLEDVal = tmpLED[2]
                if (tmpLEDNum >= 1) and (tmpLEDNum <= 9) then
                    if (tmpLEDVal == 2) then
                        tmpLEDVal = 1 - ledColors[tmpLEDNum][1]
                    elseif not ((tmpLEDVal == 1) or (tmpLEDVal == 0)) then
                        tmpLEDVal = 0
                    end
                    ledColors[tmpLEDNum][1] = tmpLEDVal
                    ledColors[tmpLEDNum][2] = 0
                    ledColors[tmpLEDNum][3] = 0
                end
            end
        end
