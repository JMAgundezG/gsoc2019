...
if __name__ == "__main__":        
    ...
    if clientID!=-1:
        ...
        res, v0= vrep.simxGetObjectHandle(clientID,'DefaultCamera',vrep.simx_opmode_oneshot_wait)
        res, v1= vrep.simxGetObjectHandle(clientID,'PassiveVision_sensor',vrep.simx_opmode_oneshot_wait)
        res, motor_left = vrep.simxGetObjectHandle(clientID, 'Motor_B', vrep.simx_opmode_streaming)
        res, motor_right = vrep.simxGetObjectHandle(clientID, 'Motor_C', vrep.simx_opmode_streaming)
        res = vrep.simxSetJointTargetVelocity(clientID, motor_left, 100, vrep.simx_opmode_streaming)
        res = vrep.simxSetJointTargetVelocity(clientID, motor_right, -100, vrep.simx_opmode_streaming)
        first = False
        count = 0
