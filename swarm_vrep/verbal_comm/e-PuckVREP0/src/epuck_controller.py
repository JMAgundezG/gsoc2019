...
class EPuckController(VRepClientController):
    ...
    def set_signal(self, signal, val):
        res = vrep.simxSetStringSignal(self.client_id, signal, val, vrep.simx_opmode_blocking)
        if res != 0:
            err_print("SET SIGNAL", parse_error(res))
            raise Exception("ERROR IN SET SIGNAL")
    
    def get_signal(self, signal):
        res, signal_val = vrep.simxGetStringSignal(self.client_id, signal, vrep.simx_opmode_blocking)
        if res <= 8:
            return signal_val if signal_val != '' else '0'
        else:
            err_print("GET SWARM DATA", parse_error(res))
    ...
    def lights_on(self, color):
        self.set_signal(self.led_name, 'ON'+str(color))
    
    def lights_off(self):
        self.set_signal(self.led_name, 'OFF')