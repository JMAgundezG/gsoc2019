...
for p in icePaths:
	if os.path.isfile(p+'/Camera.ice'):
		...
		break
if not ice_Camera:
	print 'Couln\'t load Camera'
	sys.exit(-1)
from RoboCompCamera import *
ice_CommonHead = False
for p in icePaths:
	if os.path.isfile(p+'/CommonHead.ice'):
		preStr = "-I/opt/robocomp/interfaces/ -I"+ROBOCOMP+"/interfaces/ " + additionalPathStr + " --all "+p+'/'
		wholeStr = preStr+"CommonHead.ice"
		Ice.loadSlice(wholeStr)
		ice_CommonHead = True
		break
if not ice_CommonHead:
	print 'Couln\'t load CommonHead'
	sys.exit(-1)
from RoboCompCommonHead import *
